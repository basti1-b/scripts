...
function Launch-Alltests($values) 
{
	try { 
		...
		foreach ($techCode in $AtomicTests.keys) {
            ...
            #Write-Host "Power"
		    if ($AtomicTests.$techCode.atomic_tests.executor_pwr) {
			    Write-Host "`n"$AtomicTests.$techCode.atomic_tests.executor_pwr.name -Foreground Cyan
			    if ($AtomicTests.$techCode.atomic_tests.executor_pwr.name.Contains('powershell')) {
				    foreach ($atest in $AtomicTests.$techCode.atomic_tests.executor_pwr.command -split "\n") {
					    if (-Not ([string]::IsNullOrEmpty($atest))) {
					        Write-Host "Invoking Test --> " $atest -Foreground Green
						    #Search for #{(any char(except linefeed),match 0 or more, lazy, grouped together)} string to determine if user input is required
